  ...
  function showScore() {
    quizContent.innerHTML = `
      <div class="score-screen">
        ...
        ${score === quizData.length ? '<button id="certificado-btn" class="next-btn" aria-label="Obtener certificado">Obtener Certificado</button>' : ''}
        <button id="restart-btn" class="next-btn" aria-label="Reiniciar quiz">Reiniciar Quiz</button>
      </div>
    `;
    
    if (score === quizData.length) {
      document.getElementById('certificado-btn').addEventListener('click', () => {
        window.location.href = 'certificadoFinal.html';
      });
    }
  ...
  showQuestion();
    ...
    returnBtn.addEventListener('click', () => {
      window.location.href = 'diplomados.html';
    });